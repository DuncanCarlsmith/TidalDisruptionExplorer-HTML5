...
**M6c done.** Help and About modals populated with real content from
help_text.js, 30/30 PASS:

Help modal:
- helpHeader paragraphs at top
- 'NUMERIC PARAMETERS' section with 22 entries (label / units / description)
  in same order as the GUI controls panel
- 'DIAGNOSTIC READOUTS' section with 6 entries (t, sep, COM_v, bound, E, L_z)
- Total ~10,800 chars, scrollable within modal

About modal:
- Title block + intro paragraph
- 4 section headings: PHYSICS, 2D vs 3D ROCHE LIMIT, EXPERIMENTS TO TRY, USAGE
- 24 paragraphs, 3 indented monospace formula lines
- Total ~4,800 chars

Math expressions still in plaintext form (e.g. `2.44 * R_M * (rho_M / rho_s)^(1/3)`).
M6d will wrap them as LaTeX for MathJax rendering.

Next sub-milestone: M6d — add MathJax CDN, convert formula expressions to
LaTeX so they render with proper math typography.
