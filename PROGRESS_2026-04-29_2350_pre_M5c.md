...
- [ ] **M5**: All 7 scenarios + ICs visually verified; theme toggle live-tested.
  ...
  - [ ] **M5c**: Long-running disruption case
...
**M5b done.** All 7 binary scenarios verified in headless Chromium:
- Lagrange L1: cluster at (0.00, 0.00) — at saddle between equal-mass primaries
- Lagrange L2: cluster at (59.92, 0.00) — beyond M1 along +x
- Lagrange L3: cluster at (-59.92, 0.00) — beyond M2 along -x
- Lagrange L4: cluster at (0.00, 43.30) — equilateral triangle apex (+y)
- Lagrange L5: cluster at (0.00, -43.30) — equilateral triangle apex (-y)
- Circumbinary: (62.50, 0), vy=2.1279 (ref 2.2592 * Mt/(Mt+mc) = 2.1246)
- Binary parabolic: (200, 0), vy=1.6832 (ref 1.7861 * Mt/(Mt+mc) = 1.6797)

All 7 with bound = 100% initially. Roche radii = [40.46, 40.46] for all
Lagrange cases (equal-mass M1=M2=150). Velocities match reference within
thermal-noise contribution (~0.003).

Screenshots saved in container: m5b_{lagrange_L1..L5,circumbinary,binary_parabolic}.png
L4 screenshot visually confirmed equilateral triangle geometry.

Next sub-milestone: M5c — long-running disruption case (periapsis well inside
disk Roche, run 1000+ steps, verify cluster unbinds and forms tidal tail).
