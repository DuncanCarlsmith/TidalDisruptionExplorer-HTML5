...
- [ ] **M6**: Help and About popups with MathJax math.
  ...
  - [ ] **M6c**: Wire help text into popups (plaintext)
  - [ ] **M6d**: Add MathJax for math rendering
- [ ] **M7**: Final polish, README, screenshots, FEX-style description.
- [ ] **M8**: GitHub Pages deploy verified.

## Current state

**M6b done.** Modal popup component added to index.html, 19/19 PASS:
- Modal hidden by default, shown via `Modal.show(title, content)`
- `content` accepts either string (wrapped in <p>) or DOM node (used as-is, ready
  for M6c rich content)
- Close mechanisms: X button, click outside window on overlay, Escape key
- Animation auto-pauses when modal opens, auto-resumes when modal closes
  (only if it was running before — doesn't accidentally start a paused sim)
- Theme variables work in modal so dark/light theme applies consistently
- aria-hidden / role=dialog / aria-modal attributes for accessibility

Help and About buttons currently show placeholder text. M6c will plug in
the real help_text.js content.

Next sub-milestone: M6c — wire help_text.js content into the popups as
plaintext (no math rendering yet).
