...
- [ ] **M6**: Help and About popups with MathJax math.
  ...
  - [ ] **M6b**: Generic modal popup component
...
**M6a done.** Ported `parameter_help.m` content to a new JS file `help_text.js`:
- 22 parameter entries, each with {label, units, description}
- 6 diagnostic entries (t, sep, COM_v, bound, E, L_z)
- aboutText: 49-paragraph array with PHYSICS, ROCHE 2D vs 3D, and 6
  EXPERIMENTS sections
- helpHeader: 547-char intro paragraph
- Total payload: 17 KB
- 18/18 smoke checks PASS

Math expressions kept as plaintext for now (e.g. `R_3D = R*(2*rho_M/rho_s)^(1/3)`).
Will be wrapped as LaTeX for MathJax in M6d.

No changes to `index.html` yet — the help_text.js file is just data.
M6b will add the modal popup component that displays it.

Next sub-milestone: M6b — generic modal popup component (Help and About
buttons open empty modals; verify open/close interactions work).
