...
- [ ] **M5**: All 7 scenarios + ICs visually verified; theme toggle live-tested.
  ...
  - [ ] **M5b**: Binary scenarios (lagrange L1..L5, circumbinary, parabolic)
...
**M5a done.** All 4 single-source IC families verified in headless Chromium:
- circular: cluster vy = 3.5122 (ref 3.5090 after subtractTotalCOMVelocity)
- elliptical: cluster vy = 4.3007 (ref 4.2976)
- parabolic: cluster vy = 4.9650 (ref 4.9624)
- hyperbolic: cluster vy = 4.9904 (ref 4.9868)

All match MATLAB reference within thermal-noise contribution (~0.003).
Cluster COM x=100, y=0 in every case. Roche R3D=82.030 in every case
(consistent with sourceMass=1250, particleMass=1, Nparticles=19).
All initial bound fractions = 100%. View scaling correctly differs per IC
(circular tight, elliptical wider, parabolic widest).

Screenshots saved in container: m5a_single_{circular,elliptical,parabolic,hyperbolic}.png
(not committed; visual inspection only).

Next sub-milestone: M5b — binary scenarios (lagrange L1..L5, circumbinary, parabolic).
