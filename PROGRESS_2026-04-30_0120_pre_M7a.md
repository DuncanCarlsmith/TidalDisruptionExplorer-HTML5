...
**M6 done.** Help and About modals now render LaTeX math via MathJax 3.
Full M6 sub-milestone breakdown:
- M6a: 18/18 PASS — ported parameter_help.m to help_text.js
- M6b: 19/19 PASS — modal popup component with theme support
- M6c: 30/30 PASS — help/about content wired in (plaintext)
- M6d: 13/13 PASS — MathJax 3 (CDN-loaded) typesets math

M6d details:
- MathJax 3 loaded async from cdn.jsdelivr.net (lazy, doesn't block GUI)
- Configured for \\(...\\) inline and \\[...\\] display delimiters
- `Modal.typesetMath()` defensively handles all MathJax loading states
  (works on first show even if MathJax still loading via startup.promise)
- Math typeset on every modal open, not just first

Math converted to LaTeX:
- Help: 8 inline math elements (restitution e=sqrt(1-energyLoss), Kepler
  T=2pi*sqrt(a^3/GM) twice, mass ratio mu, bound condition, L_z sum)
- About: 3 display equations (d_Roche_fluid, d_Roche_rigid, d_disk)

Total M6 verified: 80 sub-checks across 4 sub-milestones.

Next milestone: M7 — final polish. Will be broken into M7a (README update),
M7b (favicon, viewport meta, style polish), M7c (final smoke pass +
